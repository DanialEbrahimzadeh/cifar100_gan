...
def linear(x, unit, wn=False, layer_name='linear', phase=None):
    with tf.name_scope(layer_name):
        if wn :
            w_init = weight_norm(x, unit)
            x = tf.layers.dense(inputs=x, units=unit, kernel_initializer=w_init, trainable=phase)
        else :
            x = tf.layers.dense(inputs=x, units=unit, kernel_initializer=he_init, trainable=phase)
        return x

def fullyconnect(x,layer_name, num_out, phase):
    with tf.name_scope(layer_name):
        #w_init = weight_norm(x, num_out)
        x = tf.contrib.layers.fully_connected(x,num_outputs=num_out, trainable=phase)
        return x
