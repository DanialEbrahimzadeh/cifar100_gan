...
class TripleGAN(object) :
    def __init__(self, sess, epoch, batch_size, unlabel_batch_size, z_dim, dataset_name, n, gan_lr, cla_lr, checkpoint_dir, result_dir, log_dir):
        ...
        if self.dataset_name == 'cifar100' :
            ...
            self.res = resnet.ResNet101(weights='imagenet',include_top=False)

            self.data_X, self.data_y, self.unlabelled_X, self.unlabelled_y, self.labelled_features, self.unlabelled_features, self.test_X, self.test_y, self.test_features = cifar100.prepare_data(n) # trainX, trainY, testX, testY
    ...
    def resnet101(self, data):
        xx=[]
        data = data * 127.5 + 127.5
        for i in range(data.shape[0]):
            image = data[i,]
            image1 = misc.imresize(image, [224, 224])
            image1 = img_to_array(image1)
            image1 = np.expand_dims(image1, axis=0)
            feature = self.res.predict(image1)
            feature1 = feature.reshape((2048))
            xx.append(feature1)
        xx=np.asarray(xx)
        return xx
    ...
    def discriminator(self, x, y_, scope='discriminator', is_training=True, reuse=False):
        with tf.variable_scope(scope, reuse=reuse) :
            ...
            x_logit = linear(x, unit=1, layer_name=scope+'_linear1',phase='is_training')
            out = sigmoid(x_logit)
    ...
    def classifier(self, x, scope='classifier', is_training=True, reuse=False):
        with tf.variable_scope(scope, reuse=reuse) :
            ...
            #FCs = np.array([2048, 1024, 512]) # length of this array show number of FC layers, and each
            # number shows the number of neurons in that layer.
            # build FC layers that we specify in 'FCs' array
            x = fullyconnect(x, layer_name=scope + '_FC' + str(0 + 1), num_out=2048, phase=is_training)
            x = fullyconnect(x, layer_name=scope + '_FC' + str(1 + 1), num_out=1024, phase=is_training)
            x = fullyconnect(x, layer_name=scope + '_FC' + str(2 + 1), num_out=512, phase=is_training)

            x = linear(x, unit=100, layer_name=scope+'_linear1', phase=is_training)
            return x
    ...
    def build_model(self):
        ...
        self.inputs_features = tf.placeholder(tf.float32, [bs] + features_dims, name='real_features')
        self.unlabelled_inputs = tf.placeholder(tf.float32, [unlabel_bs] + image_dims, name='unlabelled_images')
        self.unlabelled_inputs_features = tf.placeholder(tf.float32, [unlabel_bs] + features_dims, name='unlabelled_features')
        self.test_inputs = tf.placeholder(tf.float32, [test_bs] + image_dims, name='test_images')
        self.test_inputs_features = tf.placeholder(tf.float32, [test_bs] + features_dims, name='test_labels')
        self.G_features = tf.placeholder(tf.float32, [bs] + features_dims, name='G_feat')
        ...
        self.G = self.generator(self.z, self.y, is_training=True, reuse=False)
        #G = self.sess.run(G)
        D_fake, D_fake_logits, _ = self.discriminator(self.G, self.y, is_training=True, reuse=True)

        # output of C for real images
        C_real_logits = self.classifier(self.inputs_features, is_training=True, reuse=False)
        ...
        Y_c = self.classifier(self.unlabelled_inputs_features, is_training=True, reuse=True)
        ...
        #G = resizing(G)
        C_fake_logits = self.classifier(self.G_features, is_training=True, reuse=True)
        ...
        test_Y = self.classifier(self.test_inputs_features, is_training=False, reuse=True)
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.epoch):
            ...
            for idx in range(start_batch_id, self.num_batches):
                ...
                batch_features = self.labelled_features[idx * self.batch_size : (idx + 1) * self.batch_size]
                ...
                batch_unlabelled_features = self.unlabelled_features[idx * self.unlabelled_batch_size : (idx + 1) * self.unlabelled_batch_size]
                ...
                # update G network
                _, summary_str_g, g_loss, GG = self.sess.run([self.g_optim, self.g_sum, self.g_loss, self.G], feed_dict=feed_dict1)
                self.writer.add_summary(summary_str_g, counter)

                features_g = self.resnet101(data=GG)

                #batch_images = resizing(batch_images)
                #unlabelled_inputs = resizing(unlabelled_inputs)
                feed_dict = {
                    self.inputs: batch_images, self.y: batch_codes,
                    self.inputs_features:batch_features,
                    self.unlabelled_inputs: batch_unlabelled_images,
                    self.unlabelled_inputs_y: batch_unlabelled_images_y,
                    self.unlabelled_inputs_features:batch_unlabelled_features,
                    self.G_features: features_g,
                    self.z: batch_z, self.alpha_p: alpha_p,
                    self.gan_lr: gan_lr, self.cla_lr: cla_lr,
                    self.unsup_weight : unsup_weight
                }
            ...
            for idx in range(10) :
                ...
                test_batch_features = self.test_features[idx * self.test_batch_size : (idx + 1) * self.test_batch_size]
                test_batch_y = self.test_y[idx * self.test_batch_size : (idx+1) * self.test_batch_size]
                #test_batch_x = resizing(test_batch_x)
                acc_ = self.sess.run(self.accuracy, feed_dict={
                    self.test_inputs: test_batch_x,
                    self.test_inputs_features: test_batch_features,
    ...
    def save(self, checkpoint_dir, step):
        ...
        prev_num = step - 2 * self.num_batches
        chk_del1 = os.path.join(checkpoint_dir, self.model_name + '.model-' + str(prev_num))
        chk_del = os.path.join(checkpoint_dir, self.model_name + '.model-' + str(prev_num) + '.meta')
        if os.path.exists(chk_del):
            os.remove(chk_del1 + '.meta')
            os.remove(chk_del1 + '.index')
            os.remove(chk_del1 + '.data-00000-of-00001')
